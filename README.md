...
Personal Electricity consumption analysis

## Installation

The source code is stored on Jfrog and requires associated login and password.
Make sure `~/.pip/pip.conf` is configured.
Use the package manager [pip](https://pip.pypa.io/en/stable/) to install electricity_consumption.

```bash
pip install electricity_consumption
```
...
- Create a Jupyter kernel for your project:
  ...
  ipython kernel install --user --name=electricity-consumption
  ```
- This kernel will now be accessible under the name `electricity-consumption` in Jupyter Lab.
...
Add your modules to the generated project's reference file at : `electricity-consumption/docs/reference.rst`
...
By default, documentation for `electricity_consumption` can be accessed with the Metroscope VPN only and at : `rnd.metroscope.io/electricity_consumption-doc`.
